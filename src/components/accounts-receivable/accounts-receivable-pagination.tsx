...
type AccountsReceivablePaginationProps = {
  query: AccountReceivableListQuery;
  total: number;
};

export function AccountsReceivablePagination({
  query,
  total,
}: AccountsReceivablePaginationProps) {
    ...
    <div className="flex items-center justify-end gap-6 px-2">
          ...
          onValueChange={(value) => {
            router.replace(
              buildAccountsReceivableListHref({
                ...query,
                pageSize: Number(value) as AccountReceivableListQuery["pageSize"],
                page: 1,
              }),
            );
          }}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {canPrevious ? (
            <Link
              href={buildAccountsReceivableListHref({ ...query, page: 1 })}
        ...
        <Button variant="outline" size="icon-sm" disabled={!canPrevious} asChild={canPrevious}>
          {canPrevious ? (
            <Link
              href={buildAccountsReceivableListHref({ ...query, page: currentPage - 1 })}
        ...
        >
          {canNext ? (
            <Link
              href={buildAccountsReceivableListHref({ ...query, page: totalPages })}
