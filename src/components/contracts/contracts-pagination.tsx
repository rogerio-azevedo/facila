...
type ContractsPaginationProps = {
  query: ContractListQuery;
  total: number;
};

export function ContractsPagination({ query, total }: ContractsPaginationProps) {
    ...
    <div className="flex items-center justify-end gap-6 px-2">
          ...
          onValueChange={(value) => {
            router.replace(
              buildContractsListHref({
                ...query,
                pageSize: Number(value) as ContractListQuery["pageSize"],
                page: 1,
              }),
            );
          }}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {canPrevious ? (
            <Link
              href={buildContractsListHref({ ...query, page: 1 })}
        ...
        <Button variant="outline" size="icon-sm" disabled={!canPrevious} asChild={canPrevious}>
          {canPrevious ? (
            <Link
              href={buildContractsListHref({ ...query, page: currentPage - 1 })}
        ...
        >
          {canNext ? (
            <Link
              href={buildContractsListHref({ ...query, page: totalPages })}
